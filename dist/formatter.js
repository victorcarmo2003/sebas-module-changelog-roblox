...
export async function formatChangelog(ai, update) {
    if (!ai) {
    ...
    const result = await ai.run({
        systemPrompt: "Voce transforma changelogs do Roblox em resumos tecnicos para Discord. Foque em mudancas impactantes para desenvolvedores.",
        userPrompt: prompt,
    ...
    return normalizeFormatted(result.json, update);
}
export function formatChangelogLocally(update) {
    return formatLocally(update);
}
export async function analyzeTopic(ai, update, topic) {
    if (!ai) {
        return analyzeTopicLocally(topic);
    }
    const result = await ai.run({
        systemPrompt: "Voce analisa um unico topico de changelog do Roblox por vez e responde em JSON estrito, sem inventar detalhes.",
    ...
    return normalizeTopicAnalysis(topic, result.json);
}
function analyzeTopicLocally(topic) {
...
function buildTopicPrompt(update, topic) {
    return [
...
function isCodeGroundedInApiRefs(code, apiRefs) {
    if (!code || apiRefs.length === 0)
        return false;
...
function normalizeTopicAnalysis(topic, output) {
...
function buildPromptPayload(update) {
...
function countIgnoredPendingReleaseNotes(update, selectedNotes) {
    if (!update.releaseNotes.some((note) => note.status === "Live"))
        return 0;
    const selected = new Set(selectedNotes.map((note) => `${note.status}:${note.text}`));
    return update.releaseNotes.filter((note) => note.status === "Pending" && !selected.has(`${note.status}:${normalizeApiMarkup(note.text)}`)).length;
}
function selectApiChangeHints(update) {
    if (update.releaseNotes.some((note) => note.status === "Live"))
        return [];
...
function scoreReleaseNote(text, status) {
    const lower = text.toLowerCase();
    let score = status === "Live" ? 8 : 2;
    if (/\b(adds?|deprecates?|removes?|now|fixes?|validates?|logs?)\b/.test(lower))
        score += 3;
    if (/(engine api|method|propert|class\.|service|getuser|groupservice|editablemesh|animation|texturecontent|controller)/i.test(text))
        score += 5;
    if (/(deprecat|remove|breaking|now only|changed from|renamed)/i.test(text))
        score += 5;
    if (/(windows accessibility|script editor|highlight|assistant|generate_mesh|generate_procedural_model)/i.test(text))
        score -= 5;
    return score;
}
function normalizeApiMarkup(value) {
    return value.replace(/`Class\./g, "`").replace(/`Library\./g, "`").replace(/\s+/g, " ").trim();
}
function buildMainPrompt(payload) {
    return [
...
function normalizeFormatted(changelog, update) {
    const formatted = {
...
function sanitizeFormatted(changelog, update) {
...
function filterUnsupportedItems(items, sourceText) {
    return items.filter((item) => !isUnsupportedClaim(item, sourceText));
}
function isUnsupportedClaim(value, sourceText) {
    ...
    if (unsupportedClaims.some(({ claim, source }) => claim.test(lower) && !source.test(sourceText)))
        return true;
    return hasIncorrectPendingClaim(lower, sourceText);
}
function hasIncorrectPendingClaim(value, sourceText) {
    if (!/pending/i.test(value) || /\blive\b/i.test(value))
        return false;
    return extractIdentifiersByStatus(sourceText, "live").some((identifier) => value.includes(identifier.toLowerCase()));
}
function shouldUseConceptualCode(codeExample, update) {
    ...
    if (extractIdentifiersByStatus(sourceText, "pending").some((identifier) => code.includes(identifier.toLowerCase())))
        return true;
    if (extractApiTokensByStatus(update, "Pending").some((token) => code.includes(token.toLowerCase())))
        return true;
...
export function buildConceptualCodeExample(update) {
...
function buildReleaseOverviewDescription(update) {
    ...
    const pendingText = liveCount > 0 && pendingCount > 0
...
function buildReleaseFootnote(update) {
    const liveCount = update.releaseNotes.filter((note) => note.status === "Live").length;
    const pendingCount = update.releaseNotes.filter((note) => note.status === "Pending").length;
    if (liveCount > 0 && pendingCount > 0)
        return `${pendingCount} item(ns) Pending foram omitidos.`;
    return undefined;
}
export function versionLabel(update) {
    return update.versionNumber ? `v${update.versionNumber}` : update.title;
}
function buildOfficialHighlights(update) {
    const selected = [];
...
function formatReleaseNoteHighlight(note) {
    ...
    if (/getrolesingroupasync/i.test(text))
        return `[${status}] \`GroupService:GetRolesInGroupAsync(userId, groupId)\` lista todos os roles e depreca os helpers antigos de \`Player\`.`;
    if (/texturecontent|meshcontent|animationcontent|charactermesh/i.test(text))
        return `[${status}] Novos campos de conteúdo para animações, meshes, beams e partículas.`;
    if (/editablemesh:clear/i.test(text))
        return `[${status}] \`EditableMesh:Clear()\` limpa vértices, bones e FACS sem remover o objeto.`;
    if (/groundcontroller/i.test(text))
        return `[${status}] \`GroundController.AccelerationTime\` e \`DecelerationTime\` passam a afetar só aceleração linear.`;
    return `[${status}] ${stripMarkdown(text).slice(0, 180)}`;
}
function extractApiTokensByStatus(update, status) {
...
function extractIdentifiersByStatus(sourceText, status) {
...
function extractAmbiguousMethods(update) {
    return update.releaseNotes.flatMap((note) => {
        const matches = [...note.text.matchAll(/`(?:Class\.)?[A-Za-z]\w*:(?<method>[A-Za-z]\w*)\(\)`/g)];
        return matches.map((match) => match.groups?.["method"]).filter((method) => Boolean(method));
    });
}
function extractFirstMethodReference(text) {
...
function normalizeList(value) {
    if (!Array.isArray(value))
        return [];
    return value.filter((item) => typeof item === "string").map(stripMarkdown).filter(Boolean);
}
