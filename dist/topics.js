...
const STATUS_PRIORITY = { Live: 0, Changed: 1, Deprecated: 2, Removed: 3, Pending: 4, Unknown: 5 };
const CONFIDENCE_PRIORITY = { high: 0, medium: 1, low: 2 };
const BREAKING_STATUSES = new Set(["Removed", "Deprecated", "Changed"]);
export function splitChangelogIntoTopics(update, maxTopics = DEFAULT_MAX_TOPICS) {
...
export function aggregateTopicAnalyses(update, analyses, meta) {
    ...
    const advantages = dedupe(ordered.filter((topic) => topic.status === "Live" || topic.status === "Changed").map((topic) => topic.developerImpact)).slice(0, 4);
    ...
    const footnoteParts = [];
    if (!codeSource)
        footnoteParts.push("Exemplo conceitual: nenhum topico Live trouxe um exemplo seguro.");
    if (meta.completedTopics < meta.totalTopics)
        footnoteParts.push(`${meta.completedTopics}/${meta.totalTopics} topicos analisados com IA a tempo.`);
    return {
        title,
        description,
        highlights,
...
function buildDescription(primary, secondary) {
...
function orderTopicsByPriority(analyses) {
...
function buildTopicTitle(text, apiRefs) {
    if (apiRefs.length > 0)
        return apiRefs[0];
    return stripMarkdown(text).split(/\s+/).slice(0, 8).join(" ");
}
function extractApiRefsFromText(text) {
    const refs = new Set();
