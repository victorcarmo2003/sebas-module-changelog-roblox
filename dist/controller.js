...
async function handleRequest(ctx, req) {
    ...
    if (parts[0] === "guilds" && parts.length === 2) {
        ...
        if (req.method === "PUT") {
            const body = req.body;
            if (!body?.channelId)
                return json(400, { error: "channelId is required" });
    ...
    if (parts.length === 1 && parts[0] === "settings") {
        if (req.method === "GET")
            return json(200, await loadModuleSettings(ctx));
        if (req.method === "PUT") {
            const body = req.body;
            const settings = {
                rssUrl: body?.rssUrl?.trim() || null,
                maxItemsPerPoll: body?.maxItemsPerPoll !== undefined && body.maxItemsPerPoll !== null && Number.isFinite(body.maxItemsPerPoll)
    ...
    if (req.method === "POST" && parts.length === 3 && parts[0] === "history" && parts[2] === "repost") {
        ...
        if (!row.versionNumber)
            return json(400, { error: "this changelog has no version number to re-fetch" });
        try {
...
async function selfTest(ctx) {
    ...
    const value = await ctx.storage.get("__selftest__");
    if (!value?.at) {
