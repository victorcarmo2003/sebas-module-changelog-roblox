...
async function handleSetup(ctx, interaction) {
    ...
    try {
        ...
        await editReply(ctx, interaction, `Nao consegui enviar mensagens em <#${channelId}>. Confira se o bot esta no servidor e tem permissoes de Ver Canal e Enviar Mensagens. Erro: ${msg.slice(0, 500)}`);
        return;
    }
    ...
    await editReply(ctx, interaction, `Setup concluido. Vou postar changelogs automaticos em <#${channelId}>. Os itens atuais foram marcados como vistos. Configure o cargo de mencao no painel, se quiser.`);
}
async function handleTeste(ctx, interaction) {
...
async function handleCommand(ctx, commandName, interaction) {
    ...
    try {
        ...
        await editReply(ctx, typed, `Nao consegui processar: ${error instanceof Error ? error.message : String(error)}`.slice(0, 1500)).catch(() => undefined);
    }
}
export default {
    commands: [
