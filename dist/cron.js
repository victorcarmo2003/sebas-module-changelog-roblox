...
async function processGuildUpdates(ctx, guildId, guildConfig, updates, maxItemsPerPoll) {
...
export async function formatWithFallback(ctx, update) {
    const ai = ctx.ai;
    if (!ai) {
        return { changelog: formatChangelogLocally(update), usedFallback: true };
    }
    const producer = () => runTopicPipeline(ai, update, { perTopicTimeoutMs: TOPIC_TIMEOUT_MS, maxTotalMs: PIPELINE_MAX_MS }).then((result) => result.changelog);
    let timeoutId;
    ...
    const errorText = result.status === "timeout"
